...
const checkNameHasFeature = (name: string) => name?.includes('feature') ?? false;
...
export default async function (host: Tree, schema: Schema) {
  const name = checkNameHasFeature(schema.name)
    ? schema.name
    : `feature-${sanitizeName(schema.name)}`;

  const libScheme: LibrarySchema = {
    ...schema,
    name,
    tags: mergeTags(schema.tags, `scope:${schema.directory},type:feature`),
  };
